...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    rtb_DeadZonex = (heli_q8_DW.clockTickCounter < heli_q8_P.PulseGenerator_Duty)
    ...
    /* Gain: '<Root>/Gain' */
    rtb_Gain = heli_q8_P.Gain_Gain * rtb_DeadZonex;

    /* DiscretePulseGenerator: '<Root>/Pulse Generator1' */
    rtb_DeadZonex = (heli_q8_DW.clockTickCounter_j <
                     heli_q8_P.PulseGenerator1_Duty) &&
      (heli_q8_DW.clockTickCounter_j >= 0) ? heli_q8_P.PulseGenerator1_Amp : 0.0;
    if (heli_q8_DW.clockTickCounter_j >= heli_q8_P.PulseGenerator1_Period - 1.0)
    {
      heli_q8_DW.clockTickCounter_j = 0;
    } else {
      heli_q8_DW.clockTickCounter_j++;
    }

    /* End of DiscretePulseGenerator: '<Root>/Pulse Generator1' */

    /* Sum: '<Root>/Sum1' */
    heli_q8_B.Sum1 = rtb_Gain + rtb_DeadZonex;
    ...
    heli_q8_B.Gain[0] += heli_q8_P.F_aug[0] * heli_q8_B.Sum1;
    heli_q8_B.Gain[0] += heli_q8_P.F_aug[2] * heli_q8_B.Joystick_gain_y;
    heli_q8_B.Gain[1] = 0.0;
    heli_q8_B.Gain[1] += heli_q8_P.F_aug[1] * heli_q8_B.Sum1;
  ...
  tmp[3] = heli_q8_P.Gain2_Gain * heli_q8_X.Integrator_CSTATE;
  tmp[4] = heli_q8_P.Gain3_Gain * heli_q8_X.Integrator1_CSTATE;
  ...
  for (i = 0; i < 2; i++) {
    rtb_DeadZonex = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_DeadZonex += heli_q8_P.K_aug[(i_0 << 1) + i] * tmp[i_0];
    }

    rtb_Sum1[i] = heli_q8_B.Gain[i] - rtb_DeadZonex;
  }
  ...
  /* Sum: '<S7>/Sum2' */
  rtb_DeadZonex = heli_q8_B.Constant2 + rtb_Sum1[0];

  /* Gain: '<S1>/Front gain' incorporates:
   *  Sum: '<S1>/Add'
   */
  heli_q8_B.Frontgain = (rtb_DeadZonex - rtb_Sum1[1]) * heli_q8_P.Frontgain_Gain;

  /* Gain: '<S1>/Back gain' incorporates:
   *  Sum: '<S1>/Subtract'
   */
  heli_q8_B.Backgain = (rtb_DeadZonex + rtb_Sum1[1]) * heli_q8_P.Backgain_Gain;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
  ...
  /* Gain: '<S10>/K_ed' */
  rtb_Gain = heli_q8_P.K_ed_Gain * heli_q8_B.ElevationTransferFcn;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
  ...
  heli_q8_B.Gain1 = ((((heli_q8_X.Integrator_CSTATE_n + heli_q8_B.K_ep) -
                       rtb_Gain) + heli_q8_B.Constant3) - heli_q8_B.Constant) *
    heli_q8_P.k_2;
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    /* Sum: '<S7>/Sum' */
    heli_q8_B.Sum = heli_q8_B.PitchCounttorad - heli_q8_B.Sum1;
  }
...
void heli_q8_initialize(void)
{
  ...
  heli_q8_DW.clockTickCounter_j = -15000;
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (367302154U);
  heli_q8_M->Sizes.checksums[1] = (1669112720U);
  heli_q8_M->Sizes.checksums[2] = (1968471759U);
  heli_q8_M->Sizes.checksums[3] = (3939691151U);
  ...
  heli_q8_M->Sizes.numBlockPrms = (183);/* Sum of parameter "widths" */
  return heli_q8_M;
}
